<?php
  ...
  $sql = "select j.jobID, j.stage, j.phdefJobID, j.dataset, j.variable, j.startDate, j.endDate, j.ineqOperator, j.ineqValue, j.algorithm, j.warmerToggle, j.warmerValue, TRIM(REPLACE(REPLACE(j.description, CHAR(13), ''), CHAR(10), '')) as description, j.status, j.submitTime, ST_AsText(coords) from jobs j where j.jobID IN (select j.jobID from users u, jobs j where (UPPER(u.firstname) = ? and UPPER(u.lastname) = ?) and (j.userId = u.userID)) order by j.jobID";
